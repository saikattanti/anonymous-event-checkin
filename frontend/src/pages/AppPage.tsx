    ...
    <main className="app">
      <header className="app__navbar">
        <div className="navbar__brand">
          <Link to="/" className="navbar__back" title="Back to Landing Page">←</Link>
          <h2>Anonymous Check-in</h2>
        </div>
        <div className="navbar__meta">
          <span className="badge">{networkLabel(config.network)}</span>
          <span className="badge badge--muted" title={config.contractAddress || ''}>
            {config.contractAddress
              ? `${config.contractAddress.slice(0, 8)}…${config.contractAddress.slice(-4)}`
              : 'No contract'}
          </span>
        </div>
      </header>
